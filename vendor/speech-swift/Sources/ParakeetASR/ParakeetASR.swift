...
///
/// Modified for Conure: decoder and joint networks pinned to `.cpuOnly` to work around
/// E5RT kernel IOSurface exhaustion on delegated compute units during long batch runs.
/// Original: https://github.com/soniqo/speech-swift (Apache-2.0)
///
...
public class ParakeetASRModel {
    ...
    public static func fromPretrained(
        ...
        #endif
        // Conure patch: the TDT decode loop calls decoder/joint once per emitted
        // token. Delegating those tiny graphs to the ANE/GPU leaks IOSurface-backed
        // buffers per prediction and exhausts E5RT after ~10k steps on long files.
        // Pin them to CPU; the encoder (the expensive part) keeps ANE/GPU.
        let smallComputeUnits: [MLComputeUnits] = [.cpuOnly]
        ...
        let (decoder, _) = try loadCoreMLModelWithFallback(
            name: "decoder", from: resolvedCacheDir, computeUnitsToTry: smallComputeUnits)
        progressHandler?(0.95, "Loading joint network...")
        let (joint, _) = try loadCoreMLModelWithFallback(
            name: "joint", from: resolvedCacheDir, computeUnitsToTry: smallComputeUnits)
